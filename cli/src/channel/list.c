...
static int channel_list_subresponse_ok(client_t *client, int argc, char **argv)
{
    (void)(argc);
    (void)(argv);
    ...
    while ((argcr >= 3) && ((strcmp(argvr[2], "END") != 0))) {
        client_team_print_channels(argvr[0], argvr[1], argvr[2]);
        arrfree((void **)(argvr), free);
        client_receive(client, &argcr, &argvr);
    }
    arrfree((void **)(argvr), free);
...
int channel_list_subresponse(client_t *client, int argc, char **argv)
{
    ...
    if (strcmp(argv[2], "OK") == 0)
        rc = channel_list_subresponse_ok(client, argc, argv);
    return (rc);
}
