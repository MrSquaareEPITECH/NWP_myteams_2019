...
static int thread_use_subresponse_ko(client_t *client, int argc, char **argv)
{
    (void)(client);
    (void)(argc);
    ...
    if (strstr(argv[3], "Thread doesn't exist"))
        client_error_unknown_thread(argv[4]);
    return (CODE_SUCCESS);
}
...
int thread_use_subresponse(client_t *client, int argc, char **argv)
{
    ...
    if (strcmp(argv[2], "KO") == 0)
        rc = thread_use_subresponse_ko(client, argc, argv);
    return (rc);
}
