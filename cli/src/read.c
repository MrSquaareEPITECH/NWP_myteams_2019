...
void handle_client(client_t *cli)
{
    ...
    else if (strncasecmp(cli->buffer, "/users", 6) == 0)
        send(cli->port, cli->buffer, SIZE_OF_BUFFER, 0);
    else if (strncasecmp(cli->buffer, "/user", 5) == 0)
        handle_user(cli);
    else if (strncasecmp(cli->buffer, "/use", 4) == 0)
        send(cli->port, cli->buffer, SIZE_OF_BUFFER, 0);
    else
        display_help();
}
...
void check_command(client_t *cli)
{
    if (strncasecmp(cli->buffer, "/help", 5) == 0)
        ...
        handle_send(cli);
    else if (strncasecmp(cli->buffer, "/subscribed", 12) == 0)
        send(cli->port, cli->buffer, SIZE_OF_BUFFER, 0);
